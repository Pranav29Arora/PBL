...
export default function Signup() {
  ...
  return (
    <div className="flex min-h-screen">
      <div className="relative hidden w-[42%] flex-col justify-between overflow-hidden bg-gradient-to-br from-emerald-700 via-teal-800 to-slate-950 p-10 text-white lg:flex">
        ...
        <div className="relative">
          <p className="font-display text-2xl font-bold">Join StockVision</p>
          <p className="mt-3 max-w-sm text-sm leading-relaxed text-emerald-100/90">
            Local mock auth — your data never leaves this device unless you choose to share it.
          </p>
        </div>
        <p className="relative text-xs text-emerald-200/70">Built for Indian investors · Global markets</p>
      </div>
      ...
      <div className="flex flex-1 items-center justify-center bg-slate-50 px-4 py-12 dark:bg-slate-950">
        <div className="w-full max-w-md rounded-3xl border border-slate-200/80 bg-white/95 p-8 shadow-2xl shadow-slate-300/30 backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-900/95 dark:shadow-black/40 md:p-10">
          ...
          <p className="mt-2 text-sm leading-relaxed text-slate-500 dark:text-slate-400">
            One minute setup. Stored in localStorage only.
          </p>
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div>
              <label htmlFor="name" className="mb-2 block text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                Name
              </label>
              <input
                id="name"
